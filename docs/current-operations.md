...
- **確認済み**: リポジトリ内の設定または公開エンドポイントから直接確認できた事実
- **未確認**: リポジトリと公開エンドポイントからは確定できない事項
...
本番のPHPバージョンと実行方式は未確認である。公開応答にPHPバージョンはなく、デプロイワークフローも本番サーバーへPHPを導入しない。
...
- ローカルのPHPとMySQLはDockerfileで固定されるが、本番のバージョンは記録されていない。
...
1. 稼働中コミットまたはリリースタグ
2. PHPのバージョン、Server API、主要拡張、設定ファイル
