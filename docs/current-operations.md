...
- **運用者確認済み（2026-08-08）**:
  - 製品・バージョンはMySQL 5.7.16である。
  - XServerのphpMyAdminから対象DB・テーブルを参照できる。SSHトンネル経由でSequel Aceから本番MySQLへ接続できる。
  - XServerサーバーパネルから、対象DBの手動バックアップ（非圧縮SQLまたはgz形式）を取得できる。
  - XServer側の自動バックアップから、サーバーパネル上で復元する操作がある。手動取得したSQLについては、既存のSSHトンネル経由で接続したSequel Aceの`File → Import...`から復元する経路がある。実際のrestoreは破壊的操作のため今回は実施しておらず、経路の存在確認にとどまる。
  - Sequel Aceから構造のみで全テーブルをSQL Exportし、最新mainの`docker/db/sql/*.sql`と比較した。11テーブルの名称は一致し、意味のあるスキーマ差分は`ssids.ssid`カラムのDEFAULT（本番は`DEFAULT ''`、最新mainはDEFAULTなし）の1点のみである。詳細と扱いは[`deployment.md`](deployment.md) 8.1節・8.2節を参照する。
- **リポジトリ上の構成**: Laravelの既定接続はMySQLで、時間トリガーSQLはMySQL固有関数を使用する。
- **未確認**: 本番の接続方式の詳細（TLS要否等）、タイムゾーン設定、文字コード、SQLモード、レプリケーション、gzファイルをSequel Aceへ直接Importできるか、過去のDB restore実施実績。

ローカルはMySQL Server 5.7.35であり、本番のMySQL 5.7.16とはバージョンが異なる。
...
- データベースバックアップは、XServerサーバーパネルから手動バックアップ（非圧縮SQLまたはgz形式）を取得できることを2026-08-08に運用者が確認済みである（[`deployment.md`](deployment.md) 8.1節）。
- 本番のログローテーション、外部ログ集約、死活監視、エラー通知は未確認である。
...
| データベース | MySQL Server 5.7.35 | MySQL 5.7.16（運用者確認済み、2026-08-08） |
...
| スキーマ | `docker/db/sql/*.sql`（11テーブル） | 適用方法は未確認。実スキーマはStructure-only exportで比較済みで、11テーブルの名称は一致。`ssids.ssid`のDEFAULTのみ既知差分（[`deployment.md`](deployment.md) 8.1節・8.2節） |
...
- ローカルMySQLはDockerfileで固定されており、本番データベースの製品・バージョンはMySQL 5.7.16と2026-08-08に運用者確認済みである（[`deployment.md`](deployment.md) 8.1節）。
...
4. データベースのタイムゾーン設定、文字コード、SQLモード（製品・バージョン・実スキーマは2026-08-08に運用者確認済み。[`deployment.md`](deployment.md) 8.1節・8.2節参照）
...
9. 実際のDB restoreテストによる復元確認の実績（バックアップ取得経路・復元経路の存在は2026-08-08に運用者確認済み。[`deployment.md`](deployment.md) 8.1節参照）
